...
func ParseSPSNALUnit(data []byte, parseVUIBeyondAspectRatio bool) (*SPS, error) {
	...
	reader := NewAccErrEBSPReader(rd)
	nalHdr := reader.Read(8)
	nalType := byte(nalHdr) & 0x1f
	if nalType != nalu_type_sps {
		return nil, errors.New("not sps")
	}
...
func (s *SPS) CpbDpbDelaysPresent() bool {
	...
	return s.VUI.NalHrdParametersPresentFlag || s.VUI.VclHrdParametersPresentFlag
}
...
func parseVUI(reader *AccErrEBSPReader, parseVUIBeyondAspectRatio bool) *VUIParameters {
	vui := &VUIParameters{}
	var err error
...
func parseHrdParameters(r *AccErrEBSPReader) *HrdParameters {
	hp := &HrdParameters{}
...
func readScalingList(reader *AccErrEBSPReader, sizeOfScalingList int) ScalingList {
